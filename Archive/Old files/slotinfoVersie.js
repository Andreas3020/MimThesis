      ...
      for (let k=0; k<6; k++) { //6x ONCOCHEMO (2 onco + 4 chemo)
        ...
        if(i == 6 && k == 5) {
          ...
          if(slotsTakenArray[weekNr][6*i + 42*j +k] == false) {
            ...
            core += `<td class="${weekdaysShort[i]} slotTaken" id="D${i}_H${j}_OC${k}">${slotsTakenArray[weekNr][6*i + 42*j +k].patientID}</td> </tr>`;
          }     
        } 
          ...
          if(slotsTakenArray[weekNr][6*i + 42*j+k] == false) {
            ...
            core += `<td class="${weekdaysShort[i]} slotTaken" id="D${i}_H${j}_OC${k}">${slotsTakenArray[weekNr][6*i + 42*j +k].patientID}</td>`; /* IMPLEMENT INFO PATIENT ON HOVER ?*/
          }
        }
      }    
    }

    //CHANGE GREYEDOUTHEADER (when changing weeks)
    for(let i=0; i<7; i++) {
...
function checkWeek() { 
  if ( currentWeek == weekNr) { return  "greyedOutSlot";} 
  else { return ""; } 
}

function checkDay(i, greyedOutString) {
  if(i >=  todayNr) { return "";}
  else { return greyedOutString;}
}
...
function checkPatientsPerDay() {
  if(Patient.list[tableBody.rows[1].cells[0].innerHTML].lastPatientBool == true) {
    ...
    // Only show immediatly if the currentweek is also the week that is displayed in the agenda
